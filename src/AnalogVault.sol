...
contract AnalogVault is BaseVault {
  ...
  function _strategyAddress() internal view override returns (address) {
  ...
  // === UPGRADE LOGIC ===
  // TODO: Add on-chain storage layout validation (ERC-7201 or fingerprint) when migrating to OZ 5.x.
  //       For now, storage compatibility is enforced by:
  //       1. Build-time: ./script/check-storage-compat.sh (diffs forge inspect output)
  //       2. Server-side: update-vault-factory-impl.ts (checks storageFingerprint if available)
  //       Inheritance chain MUST remain: Initializable → ReentrancyGuardUpgradeable →
  //       OwnableUpgradeable → UUPSUpgradeable → PausableUpgradeable (BaseVault)

...
contract AnalogVault is BeefyVaultConcLiq, UUPSUpgradeable {
    ...
    function initialize(
        ...
        // Note: Token approvals are not done here to avoid initialization order issues.
        // Call approveSwaps() after vault creation to approve USDC and pool tokens for swaps.
    ...
    function executeSwap(
        ...
        uint128 amt0 = pending.swapToken0;
        uint128 amt1 = pending.swapToken1;
        ...
        if (received > 0) {
            ...
            emit SwapExec(received, recipient);
        }
    }
    ...
    function upgradeToLatest() external onlyOwner {
        ...
        address latest = IAnalogVaultFactory(factory).latestImplementation();
        if (latest == address(0) || latest == old) revert InvalidImpl();
        delete pending;

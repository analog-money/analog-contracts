...
contract BeefyFeeConfigurator is Ownable {
    ...
    constructor(
        ...
        // Zero fee category initialized by default
        feeCategory[0] = FeeCategory(0, 0, 0, 0, "None", true);
    }
    ...
    function setFeeCategory(
        ...
        if (_adjust) {
            _call = (_total == 0) ? 0 : _call * DIVISOR / _total;
            _strategist = (_total == 0) ? 0 : _strategist * DIVISOR / _total;
        }
        uint256 beefy = (DIVISOR > _call + _strategist) ? DIVISOR - _call - _strategist : 0;
